...
if (true === ($a = $reg->availibilityStatus($domain))) {
    die(json_encode(array("code" => "807", "message" => "Domain is not registered, transfer not possible.", "data" => array())));
}

$price = $user->addTax($user->getDomainPrice($tld, "transfer"));
...
$authcode = isset($_REQUEST['password']) ? $_REQUEST['password'] : "";
if (empty($authcode)) {
    die(json_encode(array("code" => "810", "message" => "Authcode not given.", "data" => array())));
}

$ns = array();
for ($i = 1; $i <= 2; $i++) {
    if (empty($_REQUEST['ns' . $i])) {
        die(json_encode(array("code" => "811", "message" => "Nameserver missing ($i).", "data" => array())));
    }
...
if ($provns) {
    ...
    if (!filter_var($_REQUEST['ip'], FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)) {
        die(json_encode(array("code" => "812", "message" => "Invalid IP.", "data" => array())));
    }

    if (empty($_REQUEST['async']) && !$reg->delayDns) {
        $driver = DNSHandler::getDriver($_REQUEST['domain']);
        $driver->addZone($dom = $_REQUEST['domain'], $ns = $user->getNS());
...
if ($db->query("SELECT 1 FROM domains WHERE domain = '" . $db->real_escape_string($domain) . "' AND status = 'KK_WAITING'")->num_rows > 0) {
    die(json_encode(array("code" => "813", "message" => "Domain transfer already in system.", "data" => array())));
}
...
$reg->setUser($user);
if (empty($_REQUEST['async']) && true !== ($r = $reg->transferDomain($domain, $owner, $admin, $tech, $zone, $authcode, $ns, $privacy))) {
    die(json_encode(array("code" => "814", "message" => "Domain transfer failed.", "data" => array("detail" => $r))));
}
...
$db->query("INSERT INTO domains (`user`, `domain`, `reg_info`, `recurring`, `status`, `registrar`, `trade`, `created`, `sent`, `privacy`, `privacy_price`, `expiration`) VALUES (" . $user->get()['ID'] . ", '" . $db->real_escape_string($info['domain']) . "', '" . $db->real_escape_string(serialize($info)) . "', $recurring, 'KK_WAITING', '" . $db->real_escape_string($reg->getShort()) . "', $trade, '" . date("Y-m-d") . "', " . (empty($_REQUEST['async']) ? 1 : 0) . ", $privacy, $pp, '" . date("Y-m-d", strtotime("+1 year")) . "')");
...
$user->billDomain("<b>" . $_REQUEST['domain'] . "</b><br />Transfer<br /><br />$from - $to", $price, $_REQUEST['domain']);

$user->log("[API] [" . $_REQUEST['domain'] . "] Domain transferiert");

die(json_encode(array("code" => "100", "message" => "Domain transfer successful.", "data" => array())));
