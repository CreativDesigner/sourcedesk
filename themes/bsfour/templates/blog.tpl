<div id="content">
    <div class="container">
        ...
        <br /><br /><ul style="list-style: none; display: block; text-indent: 0; padding: 0; margin-bottom: -10px;">
          <li class="previous{if $pages <= $page} disabled{/if}" style="display: inline;"><a href="{$cfg.PAGEURL}blog/page/{$page+1}"><span aria-hidden="true">&larr;</span> {$lang.BLOG.PREV}</a></li>
          <li class="next{if $page == 1} disabled{/if}" style="float: right; display: inline;"><a href="{$cfg.PAGEURL}blog/page/{$page-1}">{$lang.BLOG.NEXT} <span aria-hidden="true">&rarr;</span></a></li>
        </ul>
        {else if $view == "post"}
        <h1>{$e.title}</h1><hr />
        <div class="blog-meta">{$e.time}{if !empty($e.admin)} &nbsp; / &nbsp; {$e.admin}{/if}{if !empty($cfg.DISQUS)}<span class="blog-comments"><i class="fa fa-comments"></i> <a href="{$cfg.PAGEURL}blog/{$e.ID}#disqus_thread">{$lang.BLOG.COMMENTS}</a></span>{/if}</div>
        ...
        <ul style="list-style: none; display: block; text-indent: 0; padding: 0; margin-bottom: -10px;">
          {if !empty($previous)}<li class="previous"><a href="{$cfg.PAGEURL}blog/{$previous.id}" style="display: inline;"><span aria-hidden="true">&larr;</span> {$previous.title}</a></li>{/if}
          {if !empty($next)}<li class="next" style="float: right; display: inline;"><a href="{$cfg.PAGEURL}blog/{$next.id}">{$next.title} <span aria-hidden="true">&rarr;</span></a></li>{/if}
        </ul>
        {/if}
