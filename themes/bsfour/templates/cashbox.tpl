<div class="container">
	...
	<div class="row" style="padding-top:20px;">
    	...
    	<div class="col-md-6">
    		<div class="card">
			  	<div class="card-body">
			    	<h5 class="card-title">{$lang.CASHBOX.PAYMENT} <small>{$lang.CASHBOX.INFORMATION}</small></h5>
			 		{if isset($msg)}{$msg}{/if}

			  		<form method="POST">
			  			<div class="form-group">
							<div class="input-group">
								{if is_object($curObj) && !empty($curObj->getPrefix())}<div class="input-group-prepend"><span class="input-group-text">{$curObj->getPrefix()}</span></div>{/if}
								...
								{if is_object($curObj) && !empty($curObj->getSuffix())}<div class="input-group-append"><span class="input-group-text">{$curObj->getSuffix()}</span></div>{/if}
							</div>
						</div>
