<div class="container">
    ...
    {if $step == "configure"}
    <div class="row" id="configure"><div class="col-md-2">&nbsp;</div><div class="col-md-8">{foreach from=$domains item=domain key=i}<div class="card panel panel-default domain_panel" data-i="{$i}" id="panel-{$i}">
      <div class="card-body panel-body"><h5 class="card-title"><span class="domain_waiting"></span><span class="domain_waiting2"></span><span id="domain-{$i}">{$domain}</span>{if $i > 0}<span class="pull-right"><a href="#" class="copy"><i class="fa fa-arrow-down"></i> {$lang.DOMAINS.COPY}</a></span>{/if}</h5>
        <div class="alert alert-danger" id="error-{$i}" style="display: none;"></div>

        <ul class="nav nav-tabs" role="tablist">
            <li class="nav-item"><a class="nav-link active" href="#owner-{$i}" aria-controls="owner-{$i}" role="tab" data-toggle="tab">{$lang.DOMAINS.OWNERC}</a></li>
            <li class="nav-item"><a class="nav-link" href="#admin-{$i}" aria-controls="admin-{$i}" role="tab" data-toggle="tab">{$lang.DOMAINS.ADMINC}</a></li>
            {if $user && $user.domain_contacts}<li class="nav-item"><a class="nav-link" href="#tech-{$i}" aria-controls="tech-{$i}" role="tab" data-toggle="tab">{$lang.DOMAINS.TECHC}</a></li>
            <li class="nav-item"><a class="nav-link" href="#zone-{$i}" aria-controls="zone-{$i}" role="tab" data-toggle="tab">{$lang.DOMAINS.ZONEC}</a></li>{else}
            <li class="nav-item"><a class="nav-link" href="#info-{$i}" aria-controls="info-{$i}" role="tab" data-toggle="tab">{$lang.DOMAINS.TECHC}/{$lang.DOMAINS.ZONEC}</a></li>{/if}
            <li class="nav-item"><a class="nav-link" href="#dns-{$i}" aria-controls="dns-{$i}" role="tab" data-toggle="tab">{$lang.DOMAINS.NSS}</a></li>
            {if array_key_exists($domain, $privacy)}<li class="nav-item"><a class="nav-link" href="#privacy-{$i}" aria-controls="privacy-{$i}" role="tab" data-toggle="tab">{$lang.DOMAINS.PRIVACY}</a></li>{/if}
            <li class="nav-item"><a class="nav-link" href="#transfer-{$i}" aria-controls="transfer-{$i}" id="tbtn-{$i}" role="tab" data-toggle="tab" style="display: none;">{$lang.DOMAINS.TRANSFER}</a></li>
        </ul>
        ...
        <br /><div class="tab-content">
            ...
            <div role="tabpanel" class="tab-pane" id="dns-{$i}">
                ...
                <span id="isp-{$i}" class="isp" data-i="{$i}"><br /><label>{$lang.DOMAINS.IP4}</label>
    ...
    <div class="card">
        <div class="card-body"><h5 class="card-title">{$lang.DOMAINS.SUGGESTIONS}</h5>
            <div class="row">
    ...
    $(".domain_panel").each(function() {
        ...
        $.get("{$cfg.PAGEURL}domains/check", {
            ...
            if(data.status == "n") {
                $("#tbtn-" + i).show();
                w.html('<span class="badge badge-warning">{$lang.DOMAINS.STATUSN}</span> <span class="badge badge-default">' + data.price + '</span> ');
                p.slideDown();
            } else if(data.status == "y") {
                w.html('<span class="badge badge-success">{$lang.DOMAINS.STATUSY}</span> <span class="badge badge-default">' + data.price + '</span> ');
                p.slideDown();
            } else {
                w.html('<span class="badge badge-danger">{$lang.DOMAINS.STATUSE}</span> ');
            }
        });
    });
    ...
    {if $step == "extensions"}
    <div class="row"><div class="col-md-2">&nbsp;</div><div class="col-md-8"><form method="POST"><div class="input-group input-group-lg input-group-box">
    ...
    <form method="POST"><input type="hidden" name="sld" id="sld" value="{$smarty.post.domain|htmlentities}" /><div class="card">
        <div class="card-body"><h5 class="card-title">{$lang.DOMAINS.POPULAR}</h5>
            {foreach from=$pricing item=i}
                ...
                <span class="tld" id="tld_{$i.tld}" data-tld="{$i.tld}">.{$i.tld}</span></label>
                <br />
            {/foreach}
        </div>
    </div><br />
    ...
    $(".tld").each(function() {
        ...
        $.get("{$cfg.PAGEURL}domains/check", {
            ...
            if(data.status == "y"){
                $("#spinner_" + tld).hide();
                $("#tld_" + tld).append(' <span class="badge badge-success">{$lang.DOMAINS.STATUSY}</span> <span class="badge badge-default">' + data.price + '</span>');
                $("#checkbox_" + tld).show();
            } else if(data.status == "n"){
                $("#spinner_" + tld).hide();
                $("#tld_" + tld).append(' <span class="badge badge-warning">{$lang.DOMAINS.STATUSN}</span> <span class="badge badge-default">' + data.price + '</span>');
                $("#checkbox_" + tld).show();
            } else {
    ...
    <form method="POST"><div class="input-group input-group-lg input-group-box">
        <input type="text" class="form-control" placeholder="{$lang.DOMAINS.CTA}" value="{if isset($smarty.post.domain)}{$smarty.post.domain|htmlentities}{/if}" name="domain">
        <span class="input-group-btn">
            <button type="submit" class="btn btn-primary">{$lang.DOMAINS.SEARCH}</button>
        </span>
...
<br /><br /><br /><div class="container"><small>{$lang.DOMAINS.PRICING_HINT}</small></div>