<?php
// Class for making stripe payments (Giropay)

class StripeGiropayPG extends PaymentGateway {
	public static $shortName = "stripe_giropay";
	...
	public function makePayment() {
		...
		if (isset($_GET['stripe_giropay'])) {
			...
			try {
				...
				$transactions->insert("stripe_giropay", $_GET['source'], $amount, $user->get()['ID'], "", 1);
				...
				$db->query("INSERT INTO gateway_logs (`data`, `log`, `time`, `gateway`) VALUES ('$data', '$log', " . time() . ", 'stripe_giropay')");
				if ($ex->getCode() == 1) {
	...
	public function getPaymentForm($amount = null) {
		...
		<script type="text/javascript">
	        document.write("<p>");
	        document.write("<form method=\"POST\" class=\"form-inline\" onsubmit=\"return false;\" id=\"giropay_form\"><div class=\"input-group\" id=\"payment_amount_group\"><?php if (!empty($c->getPrefix())) {?><span class=\"input-group-addon\"><?=$c->getPrefix();?></span> <?php }?><input type=\"text\" id=\"giropay_amount\" name=\"giropay_amount\" value=\"<?=$amount !== null ? $amount : "";?>\" placeholder=\"<?=$this->getLang('amount');?>\" onkeydown=\"stripeFeesAdded = 0;\" style=\"max-width:80px\" class=\"form-control\"><?php if (!empty($c->getSuffix())) {?> <span class=\"input-group-addon\"><?=$c->getSuffix();?></span><?php }?><\/div>&nbsp;<input value=\"<?=$this->getLang('pay') . $fees;?>\" type=\"submit\" id=\"giropay_payment\" onclick=\"addStripeFeesGiropay()\" class=\"btn btn-primary\" \/><\/form>");
	        document.write("<\/p>");
	        ...
	        function stripeResponseHandlerGiropay(status, response) {
	        	window.location = response.redirect.url;
			}

	        stripeFeesAddedGiropay = 0;
	        function addStripeFeesGiropay() {
	        	<?php if ($this->settings['excl'] == 1) {?>
	        	if(!stripeFeesAddedGiropay){
		        	var percent = <?=$this->settings['percent'];?>;
		        	var fix  	= Number(<?=$cur->convertAmount(null, $this->settings['fix']);?>);
		        	var value   = Number(document.getElementById("giropay_amount").value.replace(',', '.'));

					value += value * percent / 100;
					value += fix;
					value  = String(Number(Math.ceil(value * 100) / 100).toFixed(2));
					document.getElementById("giropay_amount").value = value<?=$CFG['NUMBER_FORMAT'] == "de" ? ".replace('.', ',')" : "";?>;
					stripeFeesAddedGiropay = 1;
				}
				<?php }?>
				...
				Stripe.source.create({
				  type: 'giropay',
				  amount: $("#giropay_amount").val().replace(",", ".") * 100,
				  currency: 'eur',
				  owner: {
				    name: '<?=$user->get()['name'];?>',
				  },
				  redirect: {
				    return_url: '<?=$CFG['PAGEURL'];?>credit?stripe_giropay=1&amount=' + ($("#giropay_amount").val().replace(",", ".") * 100),
				  },
				}, stripeResponseHandlerGiropay);
	        }
		</script>
