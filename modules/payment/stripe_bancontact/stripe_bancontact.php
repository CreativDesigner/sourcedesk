<?php
// Class for making stripe payments (Bancontact)

class StripeBancontactPG extends PaymentGateway {
	public static $shortName = "bancontact_bancontact";
	...
	public function makePayment() {
		...
		if (isset($_GET['bancontact_bancontact'])) {
			...
			try {
				...
				$transactions->insert("bancontact_bancontact", $_GET['source'], $amount, $user->get()['ID'], "", 1);
				...
				$db->query("INSERT INTO gateway_logs (`data`, `log`, `time`, `gateway`) VALUES ('$data', '$log', " . time() . ", 'bancontact_bancontact')");
				if ($ex->getCode() == 1) {
	...
	public function getPaymentForm($amount = null) {
		...
		<script type="text/javascript">
	        document.write("<p>");
	        document.write("<form method=\"POST\" class=\"form-inline\" onsubmit=\"return false;\" id=\"bancontact_form\"><div class=\"input-group\" id=\"payment_amount_group\"><?php if (!empty($c->getPrefix())) {?><span class=\"input-group-addon\"><?=$c->getPrefix();?></span> <?php }?><input type=\"text\" id=\"bancontact_amount\" name=\"bancontact_amount\" value=\"<?=$amount !== null ? $amount : "";?>\" placeholder=\"<?=$this->getLang('amount');?>\" onkeydown=\"stripeFeesAdded = 0;\" style=\"max-width:80px\" class=\"form-control\"><?php if (!empty($c->getSuffix())) {?> <span class=\"input-group-addon\"><?=$c->getSuffix();?></span><?php }?><\/div>&nbsp;<input value=\"<?=$this->getLang('pay') . $fees;?>\" type=\"submit\" id=\"bancontact_payment\" onclick=\"addStripeFeesBancontact()\" class=\"btn btn-primary\" \/><\/form>");
	        document.write("<\/p>");

	        var bancontact_public = '<?=$this->settings['public_key'];?>';
	        var userId = '<?=$user->get()['ID'];?>';
	        var userMail = '<?=$user->get()['mail'];?>';
	        var bancontact_currency = '<?=$var['myCurrency'];?>';
	        var prefix = '<?=$this->getLang('prefix');?>';
	        var pagename = '<?=$CFG['PAGENAME'];?>';

	        function stripeResponseHandlerBancontact(status, response) {
	        	window.location = response.redirect.url;
			}

	        stripeFeesAddedBancontact = 0;
	        function addStripeFeesBancontact() {
	        	<?php if ($this->settings['excl'] == 1) {?>
	        	if(!stripeFeesAddedBancontact){
		        	var percent = <?=$this->settings['percent'];?>;
		        	var fix  	= Number(<?=$cur->convertAmount(null, $this->settings['fix']);?>);
		        	var value   = Number(document.getElementById("bancontact_amount").value.replace(',', '.'));

					value += value * percent / 100;
					value += fix;
					value  = String(Number(Math.ceil(value * 100) / 100).toFixed(2));
					document.getElementById("bancontact_amount").value = value<?=$CFG['NUMBER_FORMAT'] == "de" ? ".replace('.', ',')" : "";?>;
					stripeFeesAddedBancontact = 1;
				}
				<?php }?>

				Stripe.setPublishableKey(bancontact_public);
				Stripe.source.create({
				  type: 'bancontact',
				  amount: $("#bancontact_amount").val().replace(",", ".") * 100,
				  currency: 'eur',
				  owner: {
				    name: '<?=$user->get()['name'];?>',
				  },
				  redirect: {
				    return_url: '<?=$CFG['PAGEURL'];?>credit?bancontact_bancontact=1&amount=' + ($("#bancontact_amount").val().replace(",", ".") * 100),
				  },
				}, stripeResponseHandlerBancontact);
	        }
		</script>
