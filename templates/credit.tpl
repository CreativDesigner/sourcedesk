<div class="container">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="input-group" id="credit_transfer_amount_group">
              {if $currencyObj->getPrefix()|trim != ""}<span class="input-group-addon" id="basic-addon2">{$currencyObj->getPrefix()}</span> {/if}<input type="text" value="{$user_credit_f}" class="form-control" placeholder="{nfo i=100}" id="credit_transfer_amount" />{if $currencyObj->getSuffix()|trim != ""} <span class="input-group-addon" id="basic-addon2">{$currencyObj->getSuffix()}</span> {/if}
            </div>
    ...
    <div class="row">
        ...
        <div class="col-lg-4 col-md-4">
            ...
            <div class="panel panel-success" style="width: 80%; display: inline-block; float: right;">
                <div class="panel-heading">
                    <div class="row">
                        <div class="col-xs-12 text-right">
                            <div style="font-size: 40px;">{$normal_f}</div>
                            <div>{$lang.CREDIT.NORMAL}</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-lg-4 col-md-4">
            ...
            <div class="panel panel-info" style="width: 80%; display: inline-block; float: right;">
                <div class="panel-heading">
                    <div class="row">
                        <div class="col-xs-12 text-right">
                            <div style="font-size: 40px;">{$special_f}</div>
                            <div>{$lang.CREDIT.SPECIAL} <a data-toggle="popover" title="{$lang.CREDIT.SPECIAL}" data-content="{$lang.CREDIT.SPECIAL_HELP}" data-placement="left"><i class="fa fa-question"></i></a></div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="panel-group" role="tablist" id="paygat" aria-multiselectable="true">
      {assign var=i value=0}
      {foreach from=$gateways key=gateway item=obj}
      {if $obj->isActive()}
      <div class="panel panel-default">
        <div class="panel-heading" role="tab" id="pg_head_{$i}">
          <h4 class="panel-title">
            <a data-toggle="collapse" data-parent="#paygat" href="#pg_col_{$i}" aria-expanded="true" aria-controls="pg_col_{$i}">
        ...
        <div id="pg_col_{$i}" class="panel-collapse collapse{if $i == 0} in{/if}" role="tabpanel" aria-labelledby="pg_head_{$i}">
    ...
    {if $i == 0}<p><i>{$lang.CREDIT.NO_GATEWAYS}</i></p>{else if isset($cashbox)}<small>{if $cashbox != "locked"}{if $cashbox == "inactive"}<a href="{$cfg.PAGEURL}credit/cashbox/1" class="btn btn-primary btn-xs">{$lang.CREDIT.CASHBOX_ACTIVATE}</a>{else}{$lang.CREDIT.CASHBOX_LINK}: <a href="{$cashbox}" target="_blank">{$cashbox}</a> <a href="{$cfg.PAGEURL}credit/cashbox/0" class="btn btn-primary btn-xs">{$lang.CREDIT.CASHBOX_DEACTIVATE}</a>{/if}{else}<font color="red">{$lang.CREDIT.CASHBOX_LOCKED}</font>{/if}</small>{/if}
</div>
