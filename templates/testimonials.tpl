<div id="content">
  <div class="container">
      ...
      {else}{if $testimonials|@count > 0}<div class="row" style="margin-bottom: 20px;">
        <div class="col-sm-6">
          <ul class="nav nav-pills">
            <li{if $order == "time"} class="active"{/if}><a href="{$cfg.PAGEURL}testimonials/{$page}/time">{$lang.TESTIMONIALS.TIME}</a></li>
            <li{if $order == "rating"} class="active"{/if}><a href="{$cfg.PAGEURL}testimonials/{$page}/rating">{$lang.TESTIMONIALS.SS}</a></li>
          </ul>
        </div>
      ...
      {if $id % 2 == 0}<div class="row">{assign "row_open" "1"}{/if}
        <div class="col-md-6">
          <div class="panel panel-{if isset($user.ID) && $testimonial->getAuthor(0) == $user.ID}primary{else}default{/if}">
            <div class="panel-heading">
              <h3 class="panel-title">{$testimonial->getSubject()|htmlentities} {assign var="rating" value=$testimonial->getRating()}{for $i=1 to 5}{if $rating >= $i}<i class="fa fa-star"></i>{/if}{/for}{for $i=$rating to 4}<i class="fa fa-star-o"></i>{/for} <small{if isset($user.ID) && $testimonial->getAuthor(0) == $user.ID} style="color: white;"{/if}>{dfo d=$testimonial->getTimestamp() m=0}</small></h3>
            </div>
            <div class="panel-body">
              {$testimonial->getText()|htmlentities|nl2br}<br />
              <small style="float: right;">~ {$testimonial->getAuthor()|htmlentities}</small>
              {if $testimonial->getAnswer()}
              <br /><hr style="margin-top: 5px; margin-bottom: 10px;" />
              {$testimonial->getAnswer()|htmlentities|nl2br}<br />
              <small style="float: right;">~ {$cfg.PAGENAME|htmlentities}</small>
              {/if}
            </div>
          </div>
        </div>
      ...
      {if $testimonials|@count > 0}<div class="row">
        <div class="col-sm-6">
          <ul class="nav nav-pills">
            <li{if $order == "time"} class="active"{/if}><a href="{$cfg.PAGEURL}testimonials/{$page}/time">Nach Zeit</a></li>
            <li{if $order == "rating"} class="active"{/if}><a href="{$cfg.PAGEURL}testimonials/{$page}/rating">Nach Bewertung</a></li>
          </ul>
        </div>
        {if $pages > 1}<div class="col-sm-6">
          <nav>
