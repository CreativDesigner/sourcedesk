<div id="content">
	<div class="container">
			...
			{if $buyed != 1}<ul class="nav nav-pills nav-justified thumbnail setup-panel">
                <li class="{if $step == 1}active{/if}"><a href="{$cfg.PAGEURL}cart">
                    <h4 class="list-group-item-heading">{$lang.REGISTER.STEP1}</h4>
                    <p class="list-group-item-text" {if $step == 1}style="color:white;"{/if}>{$lang.CART.STEP1}</p>
                </a></li>
                <li class="{if $step == 2 || ($step > 2 && $smsverify)}active{else if $cartitems|count == 0}disabled{/if}"><a href="{if $cartitems|count > 0}{$cfg.PAGEURL}cart/data{else}#{/if}">
                    <h4 class="list-group-item-heading">{$lang.REGISTER.STEP2}</h4>
                    <p class="list-group-item-text" {if $step == 2}style="color:white;"{/if}>{$lang.CART.STEP2}</p>
                </a></li>
				<li class="{if $step == 3 && !$smsverify}active{else}{if empty($smarty.session.card.ok) || $cartitems|count == 0 || $smsverify}disabled{/if}{/if}"><a href="{if !empty($smarty.session.card.ok) && $cartitems|count > 0 && !$smsverify}{$cfg.PAGEURL}cart/payment{else}#{/if}">
                    <h4 class="list-group-item-heading">{$lang.REGISTER.STEP3}</h4>
                    <p class="list-group-item-text" {if $step == 3 && !$smsverify}style="color:white;"{/if}>{$lang.CART.STEP3}</p>
                </a></li>
            </ul>{/if}

            {if $step == 1}
			{if $buyed == 1}
			{if $smarty.cookies.cart_pm}
			<div class="panel panel-primary">
				<div class="panel-heading">
					<h3 class="panel-title">{$lang.CART.PAYMENT}</h3>
				</div>
				<div class="panel-body">
					{foreach from=$gateways key=gateway item=obj}
					{if $obj->isActive() && $gateway == $smarty.cookies.cart_pm}
					{$obj->getPaymentForm($smarty.cookies.cart_pa)}
					{/if}
					{/foreach}
				</div>
			</div>
			...
			<table class="table table-bordered table-striped" style="margin-bottom:0;">
				...
				<tr>
					<td width="30%">{$i.extension}{if $i.typ == "domain_reg" || $i.typ == "domain_in"}{assign var="info" value=$i.type|unserialize}{$info.domain|htmlentities} {if $i.typ == "domain_reg"}- <font class="text-primary">{$lang.CART.TYPE_REG}</font>{else}- <font class="text-primary">{$lang.CART.TYPE_IN}</font>{/if}{else}{if $i.desc}<b>{/if}{$i.name|htmlentities}{if $i.desc}</b>{/if} {if !empty($i.prepaid)}- <font class="text-primary">{$lang.CART.PREPAID}</font>{/if}{if $i.typ == "update"}- <font class="text-primary">{$lang.CART.TYPE_UPDATE}</font>{/if}{if $i.typ == "product" && $i.type == "e"}- <font class="text-primary">{$lang.CART.TYPE_SOFTWARE}</font>{/if}{if $i.typ == "bundle"}- <font class="text-primary">{$lang.CART.TYPE_BUNDLE}</font>{/if}{/if} {if !empty($i.desc)}<br />{$i.desc}{/if}</td>
					...
					<td align="right" width="25%">
						{if isset($i.oldprice) && $i.oldprice > 0 && $i.oldprice != $i.raw_amount}<s style="color:red;"><span style="color:#444444;">{$i.oldsum_f}</span></s> {/if}{$i.f_sum}{if $i.type == "h" || ($i.type == "e" && $i.billing != "onetime" && $i.billing != "")} {if $i.billing == "" || $i.billing == "onetime"}{$lang.CART.ONETIME}{else if $i.billing == "monthly"}{$lang.CART.MONTHLY}{else if $i.billing == "quarterly"}{$lang.CART.QUARTERLY}{else if $i.billing == "semiannually"}{$lang.CART.SEMIANNUALLY}{else if $i.billing == "minutely"}{$lang.CART.MINUTELY}{else if $i.billing == "hourly"}{$lang.CART.HOURLY}{else if $i.billing == "annually"}{$lang.CART.ANNUALLY}{/if}{/if}{if $i.setup_sum > 0}<br />+ {$i.f_setup_sum} {$lang.CART.SETUP}{else if $i.setup_sum < 0}<br />- {$i.f_setup_sum} {$lang.CART.DISCOUNT}{/if}
						{if $i.prorata}<br /><br />{$lang.CART.PRTD}: {$i.prorata_breakdown}<br />{$lang.CART.PRUT} {$i.prorata_date}{/if}
					</td>
				</tr>
				{foreachelse}
				<tr>
			...
			<div class="panel{if !$logged_in} with-nav-tabs{/if} panel-default">
                <div class="panel-heading">
                    {if !$logged_in}<ul class="nav nav-tabs">
                        <li{if !isset($smarty.post.customer) || $smarty.post.customer == "new"} class="active"{/if}><a href="#newcust" data-toggle="tab" onclick="$('#custh').val('new');">{$lang.CART.NEW_CUSTOMER}</a></li>
                        <li{if isset($smarty.post.customer) && $smarty.post.customer == "login"} class="active"{/if}><a href="#exicust" data-toggle="tab" onclick="$('#custh').val('login');">{$lang.CART.EXISTING_CUSTOMER}</a></li>
                    </ul>{else}
                    {$lang.CART.STEP2}
                    {/if}
                </div>
                <div class="panel-body">
                    <div class="tab-content">
                        {if !$logged_in}<div class="tab-pane fade {if !isset($smarty.post.customer) || $smarty.post.customer == "new"}in active{/if}" id="newcust" style="margin-bottom: -10px;">{/if}
                        	<div class="form-group">
                        		<div class="row">
									<div class="col-md-2">
	                        			<label for="reg_salutation">
	                        				{$lang.PROFILE.SALUTATION}
	                        			</label>
	                        			<select name="salutation" class="form-control"{if $logged_in && "salutation"|in_array:$ro_fields} readonly="readonly"{/if}>
											<option value="MALE"{if isset($smarty.post.reg_salutation) && $smarty.post.reg_salutation == "MALE"} selected=""{/if}>{$lang.PROFILE.MALE}</option>
											<option value="FEMALE"{if isset($smarty.post.reg_salutation) && $smarty.post.reg_salutation == "FEMALE"} selected=""{/if}>{$lang.PROFILE.FEMALE}</option>
											<option value="DIVERS"{if isset($smarty.post.reg_salutation) && $smarty.post.reg_salutation == "DIVERS"} selected=""{/if}>{$lang.PROFILE.DIVERS}</option>
										</select>
	                        		</div>
                        	...
                        	<div class="form-group">
                    			...
                    			<input type="text" name="reg_birthday" id="reg_birthday" class="form-control" placeholder="{dfo d="27.12.1984" m=false}" value="{if isset($smarty.post.reg_birthday)}{$smarty.post.reg_birthday|htmlentities}{else if isset($smarty.session.card.birthday) && $smarty.session.card.birthday != "0000-00-00"}{dfo d=$smarty.session.card.birthday m=false}{else if $logged_in && $user.birthday != "0000-00-00"}{dfo d=$user.birthday m=false}{/if}"{if $logged_in && "birthday"|in_array:$ro_fields} readonly="readonly"{/if}>
                        	</div>
                        	{/if}
                        ...
                        <div class="tab-pane fade {if isset($smarty.post.customer) && $smarty.post.customer == "login"}in active{/if}" id="exicust">
                        	<div class="form-group">
							    ...
							    <input type="email" class="form-control" id="login_email" placeholder="{$lang.CART.EMAILP}" value="{if !empty($smarty.post.login_email)}{$smarty.post.login_email|htmlentities}{elseif !empty($smarty.post.reg_email)}{$smarty.post.reg_email|htmlentities}{/if}" name="login_email">
							</div>
			...
			<div class="checkbox">
			    <label>
			      	...
			      	{$lang.CART.WITHDRAWAL1} <a href="{$cfg.PAGEURL}withdrawal" target="_blank">{$lang.CART.WITHDRAWAL|replace:"%p":$cfg.PAGENAME}</a>{$lang.CART.WITHDRAWAL3}
			    </label>
			</div>
			...
			<div class="checkbox">
			    <label>
			      	...
			      	{$lang.CART.WITHDRAWAL1} <a href="{$cfg.PAGEURL}privacy" target="_blank">{$lang.CART.PRIVACY|replace:"%p":$cfg.PAGENAME}</a>{$lang.CART.TERMS31}
			    </label>
			</div>
      ...
      $.post("{$cfg.PAGEURL}profile", {
        ...
        $("#verifyPhone").find("i").addClass("fa-check").removeClass("fa-spinner fa-spin");
			...
			<div class="panel panel-default">
				<div class="panel-heading">
					<h3 class="panel-title">{$lang.CART.PAYMENT_METHOD}</h3>
				</div>
				<div class="panel-body" style="padding-top: 10px; padding-bottom: 10px;">
					{if $autPay}
					{$lang.CART.AUTPAY|replace:"%p":$autPay}
					{else}
					...
					<div class="radio">
						<label>
							<input type="radio" name="payment_method" value="{$gateway|htmlentities}">
							{if $obj->getLang('frontend_name')|is_string}{$obj->getLang('frontend_name')}{else}{$obj->getLang('name')}{/if}
						</label>
					...
					{if $i == 0}{$lang.CREDIT.NO_GATEWAYS}{/if}
					{/if}
				</div>
			</div>
			{/if}
			...
			$(document).ready(function() {
				document.cookie = "cart_pm=";
				document.cookie = "cart_pa={$rest_raw|abs}";

				$("[name=payment_method]").click(function() {
					document.cookie = "cart_pm=" + $(this).val();
					$("#checkout_btn").show();
				});
			});
			</script>

			<a href="{$cfg.PAGEURL}cart/buy/{$cartitems|count}/{$sum_raw2}" id="checkout_btn" class="btn btn-primary btn-block"{if $rest_raw < 0 && !$autPay} style="display: none;"{/if}>{$lang.CART.DOORDER}</a><br />
